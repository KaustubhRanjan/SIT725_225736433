/**
 * SIT725 – 5.4D Validation Tests (MANDATORY TEMPLATE)
 *
 * HOW TO RUN: (Node.js 18+ is required)
 *   1. Start MongoDB
 *   2. Start your server (npm start or node server.js)
 *   3. node validation-tests.js
 */

const BASE_URL = process.env.BASE_URL || "http://localhost:3000";
const API_BASE = "/api/books";

// =============================
// INTERNAL STATE (DO NOT MODIFY)
// =============================

const results = [];

const coverageTracker = {
  CREATE_FAIL: 0,
  UPDATE_FAIL: 0,
  TYPE: 0,
  REQUIRED: 0,
  BOUNDARY: 0,
  LENGTH: 0,
  TEMPORAL: 0,
  UNKNOWN_CREATE: 0,
  UNKNOWN_UPDATE: 0,
  IMMUTABLE: 0,
};

// =============================
// OUTPUTS FORMAT (DO NOT MODIFY)
// =============================

function logHeader(uniqueId) {
  console.log("SIT725_VALIDATION_TESTS");
  console.log(`BASE_URL=${BASE_URL}`);
  console.log(`API_BASE=${API_BASE}`);
  console.log(`INFO|Generated uniqueId=${uniqueId}`);
}

function logResult(r) {
  console.log(
    `TEST|${r.id}|${r.name}|${r.method}|${r.path}|expected=${r.expected}|actual=${r.actual}|pass=${r.pass ? "Y" : "N"}`
  );
}

function logSummary() {
  const failed = results.filter(r => !r.pass).length;
  console.log(
    `SUMMARY|pass=${failed === 0 ? "Y" : "N"}|failed=${failed}|total=${results.length}`
  );
  return failed === 0;
}

function logCoverage() {
  console.log(
    `COVERAGE|CREATE_FAIL=${coverageTracker.CREATE_FAIL}` +
    `|UPDATE_FAIL=${coverageTracker.UPDATE_FAIL}` +
    `|TYPE=${coverageTracker.TYPE}` +
    `|REQUIRED=${coverageTracker.REQUIRED}` +
    `|BOUNDARY=${coverageTracker.BOUNDARY}` +
    `|LENGTH=${coverageTracker.LENGTH}` +
    `|TEMPORAL=${coverageTracker.TEMPORAL}` +
    `|UNKNOWN_CREATE=${coverageTracker.UNKNOWN_CREATE}` +
    `|UNKNOWN_UPDATE=${coverageTracker.UNKNOWN_UPDATE}` +
    `|IMMUTABLE=${coverageTracker.IMMUTABLE}`
  );
}

// =============================
// HTTP HELPER
// =============================

async function http(method, path, body) {
  const res = await fetch(`${BASE_URL}${path}`, {
    method,
    headers: { "Content-Type": "application/json" },
    body: body ? JSON.stringify(body) : undefined,
  });

  const text = await res.text();
  return { status: res.status, text };
}

// =============================
// TEST REGISTRATION FUNCTION
// =============================

async function test({ id, name, method, path, expected, body, tags }) {
  const { status } = await http(method, path, body);
  const pass = status === expected;

  const result = { id, name, method, path, expected, actual: status, pass };
  results.push(result);
  logResult(result);

  const safeTags = Array.isArray(tags) ? tags : [];
  safeTags.forEach(tag => {
    if (Object.prototype.hasOwnProperty.call(coverageTracker, tag)) {
      coverageTracker[tag]++;
    }
  });
}

// =============================
// STUDENT MUST MODIFY THESE
// =============================

function makeValidBook(id) {
  return {
    id,
    title: "Valid Title",
    author: "Valid Author",
    year: 2020,
    genre: "Other",
    summary: "Valid summary text that satisfies the schema rules.",
    price: "9.99"
  };
}

function makeValidUpdate() {
  return {
    title: "Updated Title",
    author: "Updated Author",
    year: 2021,
    genre: "Other",
    summary: "Updated summary text that is valid.",
    price: "10.50"
  };
}

// =============================
// REQUIRED BASE TESTS
// =============================

async function run() {
  const uniqueId = `b${Date.now()}`;
  logHeader(uniqueId);

  const createPath = API_BASE;
  const updatePath = (id) => `${API_BASE}/${id}`;

  await test({
    id: "T01",
    name: "Valid create",
    method: "POST",
    path: createPath,
    expected: 201,
    body: makeValidBook(uniqueId),
    tags: []
  });

  await test({
    id: "T02",
    name: "Duplicate ID",
    method: "POST",
    path: createPath,
    expected: 409,
    body: makeValidBook(uniqueId),
    tags: ["CREATE_FAIL"]
  });

  await test({
    id: "T03",
    name: "Immutable ID on update",
    method: "PUT",
    path: updatePath(uniqueId),
    expected: 400,
    body: { ...makeValidUpdate(), id: "b999" },
    tags: ["UPDATE_FAIL", "IMMUTABLE"]
  });

  await test({
    id: "T04",
    name: "Unknown field CREATE",
    method: "POST",
    path: createPath,
    expected: 400,
    body: { ...makeValidBook(`b${Date.now() + 1}`), hack: true },
    tags: ["CREATE_FAIL", "UNKNOWN_CREATE"]
  });

  await test({
    id: "T05",
    name: "Unknown field UPDATE",
    method: "PUT",
    path: updatePath(uniqueId),
    expected: 400,
    body: { ...makeValidUpdate(), hack: true },
    tags: ["UPDATE_FAIL", "UNKNOWN_UPDATE"]
  });

  // Additional coverage tests

  await test({
    id: "T06",
    name: "Required title on CREATE",
    method: "POST",
    path: createPath,
    expected: 400,
    body: (() => {
      const b = makeValidBook(`b${Date.now() + 2}`);
      delete b.title;
      return b;
    })(),
    tags: ["CREATE_FAIL", "REQUIRED"]
  });

  await test({
    id: "T07",
    name: "Required genre on UPDATE",
    method: "PUT",
    path: updatePath(uniqueId),
    expected: 400,
    body: { ...makeValidUpdate(), genre: "" },
    tags: ["UPDATE_FAIL", "REQUIRED"]
  });

  await test({
    id: "T08",
    name: "Type validation year as string on CREATE",
    method: "POST",
    path: createPath,
    expected: 400,
    body: { ...makeValidBook(`b${Date.now() + 3}`), year: "two thousand" },
    tags: ["CREATE_FAIL", "TYPE"]
  });

  await test({
    id: "T09",
    name: "Type validation price invalid string on UPDATE",
    method: "PUT",
    path: updatePath(uniqueId),
    expected: 400,
    body: { ...makeValidUpdate(), price: "abc" },
    tags: ["UPDATE_FAIL", "TYPE"]
  });

  await test({
    id: "T10",
    name: "Boundary year below minimum on CREATE",
    method: "POST",
    path: createPath,
    expected: 400,
    body: { ...makeValidBook(`b${Date.now() + 4}`), year: 1499 },
    tags: ["CREATE_FAIL", "BOUNDARY"]
  });

  await test({
    id: "T11",
    name: "Boundary year below minimum on UPDATE",
    method: "PUT",
    path: updatePath(uniqueId),
    expected: 400,
    body: { ...makeValidUpdate(), year: 1499 },
    tags: ["UPDATE_FAIL", "BOUNDARY"]
  });

  await test({
    id: "T12",
    name: "Temporal future year on CREATE",
    method: "POST",
    path: createPath,
    expected: 400,
    body: { ...makeValidBook(`b${Date.now() + 5}`), year: new Date().getFullYear() + 1 },
    tags: ["CREATE_FAIL", "TEMPORAL"]
  });

  await test({
    id: "T13",
    name: "Length title too short on CREATE",
    method: "POST",
    path: createPath,
    expected: 400,
    body: { ...makeValidBook(`b${Date.now() + 6}`), title: "Hi" },
    tags: ["CREATE_FAIL", "LENGTH"]
  });

  await test({
    id: "T14",
    name: "Length summary too short on UPDATE",
    method: "PUT",
    path: updatePath(uniqueId),
    expected: 400,
    body: { ...makeValidUpdate(), summary: "short" },
    tags: ["UPDATE_FAIL", "LENGTH"]
  });

  await test({
    id: "T15",
    name: "Update missing target record",
    method: "PUT",
    path: updatePath("b999999999"),
    expected: 404,
    body: makeValidUpdate(),
    tags: []
  });

  await test({
    id: "T16",
    name: "Valid update",
    method: "PUT",
    path: updatePath(uniqueId),
    expected: 200,
    body: makeValidUpdate(),
    tags: []
  });

  await test({
  id: "T17",
  name: "Invalid ID format on CREATE",
  method: "POST",
  path: createPath,
  expected: 400,
  body: { ...makeValidBook(`x123`) },
  tags: ["CREATE_FAIL", "TYPE"]
});

await test({
  id: "T18",
  name: "Author too short on CREATE",
  method: "POST",
  path: createPath,
  expected: 400,
  body: { ...makeValidBook(`b${Date.now() + 7}`), author: "AB" },
  tags: ["CREATE_FAIL", "LENGTH"]
});

await test({
  id: "T19",
  name: "Author too long on CREATE",
  method: "POST",
  path: createPath,
  expected: 400,
  body: { ...makeValidBook(`b${Date.now() + 8}`), author: "A".repeat(81) },
  tags: ["CREATE_FAIL", "LENGTH"]
});

await test({
  id: "T20",
  name: "Genre too short on CREATE",
  method: "POST",
  path: createPath,
  expected: 400,
  body: { ...makeValidBook(`b${Date.now() + 9}`), genre: "AB" },
  tags: ["CREATE_FAIL", "LENGTH"]
});

await test({
  id: "T21",
  name: "Genre too long on UPDATE",
  method: "PUT",
  path: updatePath(uniqueId),
  expected: 400,
  body: { ...makeValidUpdate(), genre: "A".repeat(41) },
  tags: ["UPDATE_FAIL", "LENGTH"]
});

await test({
  id: "T22",
  name: "Title too long on CREATE",
  method: "POST",
  path: createPath,
  expected: 400,
  body: { ...makeValidBook(`b${Date.now() + 10}`), title: "T".repeat(101) },
  tags: ["CREATE_FAIL", "LENGTH"]
});

await test({
  id: "T23",
  name: "Summary too long on CREATE",
  method: "POST",
  path: createPath,
  expected: 400,
  body: { ...makeValidBook(`b${Date.now() + 11}`), summary: "S".repeat(501) },
  tags: ["CREATE_FAIL", "LENGTH"]
});

await test({
  id: "T24",
  name: "Price zero on CREATE",
  method: "POST",
  path: createPath,
  expected: 400,
  body: { ...makeValidBook(`b${Date.now() + 12}`), price: "0" },
  tags: ["CREATE_FAIL", "BOUNDARY"]
});

await test({
  id: "T25",
  name: "Price negative on UPDATE",
  method: "PUT",
  path: updatePath(uniqueId),
  expected: 400,
  body: { ...makeValidUpdate(), price: "-5.00" },
  tags: ["UPDATE_FAIL", "BOUNDARY"]
});

await test({
  id: "T26",
  name: "Future year on UPDATE",
  method: "PUT",
  path: updatePath(uniqueId),
  expected: 400,
  body: { ...makeValidUpdate(), year: new Date().getFullYear() + 1 },
  tags: ["UPDATE_FAIL", "TEMPORAL"]
});

await test({
  id: "T27",
  name: "Missing author on CREATE",
  method: "POST",
  path: createPath,
  expected: 400,
  body: (() => {
    const b = makeValidBook(`b${Date.now()+20}`);
    delete b.author;
    return b;
  })(),
  tags: ["CREATE_FAIL", "REQUIRED"]
});

await test({
  id: "T28",
  name: "Missing summary on CREATE",
  method: "POST",
  path: createPath,
  expected: 400,
  body: (() => {
    const b = makeValidBook(`b${Date.now()+21}`);
    delete b.summary;
    return b;
  })(),
  tags: ["CREATE_FAIL", "REQUIRED"]
});

await test({
  id: "T29",
  name: "Missing price on CREATE",
  method: "POST",
  path: createPath,
  expected: 400,
  body: (() => {
    const b = makeValidBook(`b${Date.now()+22}`);
    delete b.price;
    return b;
  })(),
  tags: ["CREATE_FAIL", "REQUIRED"]
});

await test({
  id: "T30",
  name: "Genre too long on CREATE",
  method: "POST",
  path: createPath,
  expected: 400,
  body: { ...makeValidBook(`b${Date.now()+23}`), genre: "A".repeat(41) },
  tags: ["CREATE_FAIL", "LENGTH"]
});

await test({
  id: "T31",
  name: "Author too long on UPDATE",
  method: "PUT",
  path: updatePath(uniqueId),
  expected: 400,
  body: { ...makeValidUpdate(), author: "A".repeat(81) },
  tags: ["UPDATE_FAIL", "LENGTH"]
});

  const pass = logSummary();
  logCoverage();

  process.exit(pass ? 0 : 1);
}

run().catch(err => {
  console.error("ERROR", err);
  process.exit(2);
});